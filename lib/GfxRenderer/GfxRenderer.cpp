...
// See the header comment: ported from CrossInk's GfxRenderer, adapted to this file's own
// malloc/free row-buffer pattern (drawBitmap, above) instead of CrossInk's pooled scratch
// allocator, which this fork doesn't have.
void GfxRenderer::drawPerspectiveBitmap(const Bitmap& bitmap, const int x, const int y, const int w, const int hL,
  ...
  auto* rowBytes = static_cast<uint8_t*>(malloc(bitmap.getRowBytes()));
  if (!outputRow || !rowBytes) {
    LOG_ERR("GFX", "!! Failed to allocate BMP row buffers (perspective)");
    free(outputRow);
    free(rowBytes);
    return;
  }
